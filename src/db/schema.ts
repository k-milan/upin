...
export const bucketExclusions = pgTable(
  "bucket_exclusions",
  {
    bucketId: uuid("bucket_id")
      .notNull()
      .references(() => dailyBuckets.id, { onDelete: "cascade" }),
    date: date("date").notNull(),
  },
...
export const todos = pgTable("todos", {
  ...
  bucketId: uuid("bucket_id").references(() => dailyBuckets.id, {
    onDelete: "set null",
  }),
  position: integer("position").notNull().default(0),
  completed: boolean("completed").notNull().default(false),
  finishFirst: boolean("finish_first").notNull().default(false),
