...
export function useDeleteBucket() { const client = useQueryClient(); return useMutation({ mutationFn: deleteBucket, onSuccess: () => { client.invalidateQueries({ queryKey: ["daily-buckets"] }); client.invalidateQueries({ queryKey: ["todos"] }); }, onError: () => toast.error("Couldn’t delete that bucket.") }); }
