...
export function listDemoBuckets(date?: string) { const key = date ?? new Intl.DateTimeFormat("en-CA", { timeZone: "Asia/Manila" }).format(new Date()); const persistent = Object.values(bucketDays).flat().filter((bucket) => bucket.persistent && (!demoBucketEndsBefore.get(bucket.id) || key < demoBucketEndsBefore.get(bucket.id)!) && !demoBucketExclusions.has(`${bucket.id}:${key}`)); return [...demoDay(key), ...persistent.filter((bucket) => !demoDay(key).some((local) => local.id === bucket.id))].sort((a, b) => a.position - b.position); }
...
export function deleteDemoBucket(id: string, date: string, scope: "day" | "future" | "all") { const day = Object.values(bucketDays).find((buckets) => buckets.some((bucket) => bucket.id === id)); const index = day?.findIndex((bucket) => bucket.id === id) ?? -1; if (!day || index < 0) return null; const bucket = day[index]; if (bucket.persistent && scope === "day") { demoBucketExclusions.add(`${id}:${date}`); for (const todo of todos) if (todo.bucketId === id && todo.scheduledFor === date) todo.bucketId = null; } else if (bucket.persistent && scope === "future") { demoBucketEndsBefore.set(id, date); for (const todo of todos) if (todo.bucketId === id && todo.scheduledFor && todo.scheduledFor >= date) todo.bucketId = null; } else { day.splice(index, 1); demoBucketEndsBefore.delete(id); for (const todo of todos) if (todo.bucketId === id) todo.bucketId = null; } return { bucket, attachments: [] }; }
