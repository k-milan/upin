...
import { Check, ChevronLeft, ChevronRight, Download, GripVertical, Inbox, Moon, Paperclip, PawPrint, Plus, Sun, Trash2, X } from "lucide-react";
...
function TaskRow({ todo, onOpen, onDeleted }: { todo: Todo; onOpen: () => void; onDeleted: (id: string) => void }) {
  ...
  return <div ref={(node) => { taskDrag.setNodeRef(node); taskDrop.setNodeRef(node); }} style={{ transform: taskDrag.transform ? `translate3d(${taskDrag.transform.x}px, ${taskDrag.transform.y}px, 0)` : undefined }} className={cn("flex items-center gap-2 border-b border-border py-3.5 last:border-b-0", taskDrag.isDragging && "opacity-40", taskDrop.isOver && !taskDrag.isDragging && "bg-muted/60")}>
    <button type="button" {...taskDrag.listeners} {...taskDrag.attributes} aria-label={`Move ${todo.title}`} className="cursor-grab touch-none text-muted-foreground/50 hover:text-muted-foreground"><GripVertical className="size-4" /></button>
    ...
    <button type="button" onClick={onOpen} className="min-w-0 flex-1 text-left"><span className={cn("block text-[15px] leading-6", todo.completed && "text-muted-foreground line-through decoration-muted-foreground/50")}>{todo.title}</span></button>
...
function Bucket({ id, children }: { id: string; children: (dragHandle: React.ReactNode) => React.ReactNode }) {
  const { isOver, setNodeRef } = useDroppable({ id });
  const bucketDrag = useDraggable({ id: `bucket:${id}` });
  const dragHandle = <button type="button" {...bucketDrag.listeners} {...bucketDrag.attributes} aria-label="Move bucket" className="grid size-6 shrink-0 cursor-grab touch-none place-items-center rounded-[10px] text-muted-foreground/35 hover:bg-muted hover:text-muted-foreground"><GripVertical className="size-3.5" /></button>;
  return <section ref={(node) => { setNodeRef(node); bucketDrag.setNodeRef(node); }} style={{ transform: bucketDrag.transform ? `translate3d(${bucketDrag.transform.x}px, ${bucketDrag.transform.y}px, 0)` : undefined }} className={cn("rounded-2xl border border-transparent px-3 transition-colors", (isOver || bucketDrag.isDragging) && "border-primary/40 bg-accent/50", bucketDrag.isDragging && "opacity-50")}><div className="min-h-10">{children(dragHandle)}</div></section>;
}

function TaskComposer({ bucketId, day, composerId, activeComposerId, setActiveComposerId }: { bucketId?: string | null; day: string; composerId: string; activeComposerId: string | null; setActiveComposerId: (id: string | null) => void }) {
...
export function TodayList() {
  ...
  const [bucketToDelete, setBucketToDelete] = useState<{ id: string; name: string; taskCount: number } | null>(null);
  ...
  return <div className="min-h-screen md:flex"><CarryReviewDialog date={day} isToday={day === dateKey(new Date())} /><motion.section layout transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className={cn("min-w-0 flex-1 px-5 pb-16 pt-10 sm:pt-16", selectedTodo && "md:max-w-[50%]")}><div className="mx-auto w-full max-w-xl">
    ...
    <DndContext sensors={sensors} onDragEnd={moveTask}><div className="space-y-3">{isLoading ? <p className="py-5 text-sm text-muted-foreground">Opening your list…</p> : <>{buckets.map((bucket) => { const bucketTodos = grouped.get(bucket.id) ?? []; return <Bucket key={bucket.id} id={bucket.id}>{(dragHandle) => <><div className="mt-3 mb-2 flex h-9 items-center gap-1"><Input aria-label={`${bucket.name} bucket name`} defaultValue={bucket.name} onBlur={(event) => { const name = event.target.value.trim(); if (name && name !== bucket.name) updateBucket.mutate({ id: bucket.id, name }); }} className="h-9 flex-1 rounded-xl border-0 !bg-background px-3 text-xs font-semibold uppercase tracking-[0.14em] text-secondary-foreground shadow-none hover:!bg-muted/60 focus-visible:!bg-card focus-visible:ring-1" /><div className="flex shrink-0 items-center gap-1"><Button type="button" variant="ghost" size="icon-xs" onClick={() => setBucketToDelete({ id: bucket.id, name: bucket.name, taskCount: bucketTodos.length })} aria-label={`Delete ${bucket.name} bucket`} className="text-muted-foreground hover:text-destructive"><Trash2 className="size-3.5" /></Button>{dragHandle}</div></div>{bucketTodos.length === 0 && <p className="px-1 py-2 text-xs text-muted-foreground/70">Nothing here yet.</p>}{bucketTodos.map((todo) => <TaskRow key={todo.id} todo={todo} onOpen={() => setSelectedTodo(todo)} onDeleted={(id) => setSelectedTodo((current) => current?.id === id ? null : current)} />)}<TaskComposer bucketId={bucket.id} day={day} composerId={bucket.id} activeComposerId={activeComposerId} setActiveComposerId={setActiveComposerId} /></>}</Bucket>; })}<Bucket id="unbucketed">{(dragHandle) => <><div className="mt-4 flex items-center justify-between"><p className="px-3 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Unbucketed</p>{dragHandle}</div>{unbucketedTodos.length === 0 && <p className="px-3 py-2 text-xs text-muted-foreground/70">No loose tasks.</p>}{unbucketedTodos.map((todo) => <TaskRow key={todo.id} todo={todo} onOpen={() => setSelectedTodo(todo)} onDeleted={(id) => setSelectedTodo((current) => current?.id === id ? null : current)} />)}<TaskComposer day={day} composerId="unbucketed" activeComposerId={activeComposerId} setActiveComposerId={setActiveComposerId} /></>}</Bucket></>}</div></DndContext>
    <p className="mt-8 text-center text-xs text-muted-foreground">Drag a task by its handle, or open it for Markdown details.</p>
  </div></motion.section><AnimatePresence>{selectedTodo && <TaskDetailsPanel key="task-details" todo={selectedTodo} onClose={() => setSelectedTodo(null)} />}</AnimatePresence><Dialog open={Boolean(bucketToDelete)} onOpenChange={(open) => { if (!open) setBucketToDelete(null); }}><DialogContent><DialogHeader><DialogTitle>Delete “{bucketToDelete?.name}”?</DialogTitle><DialogDescription>{bucketToDelete?.taskCount ? `This bucket contains ${bucketToDelete.taskCount} task${bucketToDelete.taskCount === 1 ? "" : "s"}. Choose what happens to ${bucketToDelete.taskCount === 1 ? "it" : "them"}.` : "This bucket has no tasks."}</DialogDescription></DialogHeader><DialogFooter><Button type="button" variant="outline" onClick={() => setBucketToDelete(null)}>Cancel</Button>{bucketToDelete?.taskCount ? <Button type="button" disabled={deleteBucket.isPending} onClick={() => deleteBucket.mutate({ id: bucketToDelete.id }, { onSuccess: () => setBucketToDelete(null) })}>Move to Unbucketed</Button> : null}<Button type="button" variant="destructive" disabled={deleteBucket.isPending} onClick={() => bucketToDelete && deleteBucket.mutate({ id: bucketToDelete.id, deleteTasks: true }, { onSuccess: () => setBucketToDelete(null) })}>{bucketToDelete?.taskCount ? "Delete bucket and tasks" : "Delete bucket"}</Button></DialogFooter></DialogContent></Dialog></div>;
}
