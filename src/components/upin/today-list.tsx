...
export function TodayList() {
  ...
  return <div className="min-h-screen md:flex"><CarryReviewDialog date={day} isToday={day === dateKey(new Date())} /><motion.section layout transition={{ duration: 0.28, ease: [0.22, 1, 0.36, 1] }} className={cn("min-w-0 flex-1 px-5 pb-16 pt-10 sm:pt-16", selectedTodo && "md:max-w-[50%]")}><div className="mx-auto w-full max-w-xl">
    ...
    <DndContext sensors={sensors} onDragEnd={moveTask}><div className="space-y-3">{isLoading ? <p className="py-5 text-sm text-muted-foreground">Opening your list…</p> : <>{buckets.map((bucket) => { const bucketTodos = grouped.get(bucket.id) ?? []; return <Bucket key={bucket.id} id={bucket.id}><Input aria-label={`${bucket.name} bucket name`} defaultValue={bucket.name} onBlur={(event) => { const name = event.target.value.trim(); if (name && name !== bucket.name) updateBucket.mutate({ id: bucket.id, name }); }} className="mt-3 mb-2 h-9 w-full rounded-xl border-0 !bg-background px-3 text-xs font-semibold uppercase tracking-[0.14em] text-secondary-foreground shadow-none hover:!bg-muted/60 focus-visible:!bg-card focus-visible:ring-1" />{bucketTodos.length === 0 && <p className="px-1 py-2 text-xs text-muted-foreground/70">Nothing here yet.</p>}{bucketTodos.map((todo) => <TaskRow key={todo.id} todo={todo} onOpen={() => setSelectedTodo({ ...todo, checklist: todo.checklist ?? [] })} />)}<TaskComposer bucketId={bucket.id} day={day} /></Bucket>; })}<Bucket id="unbucketed"><p className="mt-4 px-3 text-xs font-semibold uppercase tracking-[0.14em] text-muted-foreground">Unbucketed</p>{unbucketedTodos.length === 0 && <p className="px-3 py-2 text-xs text-muted-foreground/70">No loose tasks.</p>}{unbucketedTodos.map((todo) => <TaskRow key={todo.id} todo={todo} onOpen={() => setSelectedTodo({ ...todo, checklist: todo.checklist ?? [] })} />)}<TaskComposer day={day} /></Bucket></>}</div></DndContext>
